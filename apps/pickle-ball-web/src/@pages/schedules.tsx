...
import { CreateShootout } from '../@components/widgets/createShootout';
...
export function Schedule() {
  ...
  const getScheduleStatus = async (args: any) => {
    const response = await fetch(
      `https://acepicklapi.raganindustries.com/api_get_schedule_status.php`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  useEffect(() => {
    ...
    if (modalElement) {
      ...
      modalElement_createShootout.addEventListener('hidden.bs.modal', (event) => {
        // Your logic goes here...
        if (isShootOut) {
          setIsShootOut(false);
        }
  ...
  }, [isUpdateScore, isMatchDetails, isPlayerLists, isShootOut]);
          ...
          {showShootOut && (
            <div className="col-6">
              <div className="d-flex justify-content-end py-3">
                <button className="btn btn-primary me-2" onClick={(e)=>showCreateShootout()}>
                  Create Shootout
                </button>
                <button
